    ...
    <div className="fixed inset-0 -z-50 overflow-hidden pointer-events-none">
      ...
      <div className="absolute inset-0">
        {animationData.binaryRain.map((item, i) => (
          <div
            key={i}
            className="absolute text-blue-400/40 text-sm animate-binary-fall select-none"
            style={{
              left: `${item.left}%`,
        ...
        {animationData.secondaryBinary.map((item, i) => (
          <div
            key={`sec-${i}`}
            className="absolute text-sky-400/30 text-xs animate-binary-fall select-none"
            style={{
              left: `${item.left}%`,
              animationDelay: `${item.delay}s`,
              animationDuration: `${item.duration}s`,
              fontFamily: 'Courier New, monospace',
              fontWeight: 'bold',
              textShadow: '0 0 6px #0ea5e9, 0 0 12px #0ea5e9, 0 0 18px #0ea5e9',
              letterSpacing: '0.2em'
            }}
          >
            {item.char}
